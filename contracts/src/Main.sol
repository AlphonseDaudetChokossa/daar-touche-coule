...
import './Ship.sol';
import './SmartShip.sol';
import './SimpleShip.sol';
...
contract Main {
  ...
  function registerNewShip(uint shipId) external{
    if(shipId==1){
          Ship smartShip = new SimpleShip();
          register(address(smartShip));
    }else if (shipId==2){
         Ship smartShip = new SmartShip();
          register(address(smartShip));
    }
